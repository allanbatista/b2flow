class ApiSdl
  ...
  def route(path)
    ...
    _path = File.join(@arguments[:path], path) if path.present?
  ...
  def params(params)
    ...
    _params = @arguments[:params].merge(params) if params.present?
  ...
  def headers(headers)
    ...
    _headers = @arguments[:headers].merge(headers) if headers.present?
  ...
  def body(body)
    ...
    _body = @arguments[:body].merge(body) if body.present?

    ApiSdl.new(connection, @arguments.merge({body: _body}))
  end

  def final_uri
    [@arguments[:path], @arguments[:params]].filter(&:present?).join("?")
  end
