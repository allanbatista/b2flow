...
RSpec.describe Job, type: :model do
  ...
  context ".create" do
    it "should create a new job" do
      job = Job.create(name: "MyJob", team: team, project: project, engine: "docker", cron: "* * * * *")

      expect(job).to be_persisted
      expect(job.name).to eq("myjob")
      expect(job.team).to eq(team)
    ...
    it "should not create two jobs with same name for same project" do
      job = Job.create(name: "MyJob", team: team, project: project, engine: "docker")
      expect(job).to be_persisted

      job2 = Job.create(name: "MyJob", team: team, project: project, engine: "docker")
